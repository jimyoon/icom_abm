...
start_year = 2018
n_years = 79  # no of years (model will run for n+1 years)
...
pop_filename = 'balt_block_group_population_2018.csv'  # accommodates census data in IPUMS/NHGIS and imported as csv
pop_fieldname = 'AJWME001'  # from IPUMS/NHGIS metadata
flood_filename = 'block_group_perc_100yr_flood.csv'  # FEMA 100-yr flood area data (see pre_"processing/flood_risk_calcs.py")
housing_filename = 'block_group_housing_1993.csv'  # housing characteristic data and other information from early 90s (for initialization)
hedonic_filename = 'simple_anova_hedonic_without_flood_block_group0418.csv'  # simple ANOVA hedonic regression conducted by Alfred
...
s = ICOMSimulator(network=None, record_time=False, progress=False, max_iterations=1,
                  name=simulation_name, scenario=scenario, intervention=intervention, start_year=start_year, n_years=n_years)
s.set_timestep_information()  # sets up timestep information based on model options (start_year, n_years)
...
# for block_group in s.network.nodes:
#     if block_group.county == '005':
#         s.network.get_institution('zoning_manager_005').add_node(block_group)
...
block_group_sample_size = 10  # the number of homes that a new agent samples for residential choice
s.add_engine(NewAgentLocation(target, block_group_sample_size, house_choice_mode=house_choice_mode, simple_anova_coefficients=simple_anova_coefficients, budget_reduction_perc=budget_reduction_perc))

# Load existing agent re-location engine to simulation object
target = s.network
block_group_sample_size = 10  # the number of homes that a re-locating agent samples for residential choice
s.add_engine(ExistingAgentLocation(target, block_group_sample_size=block_group_sample_size, house_choice_mode=house_choice_mode, simple_anova_coefficients=simple_anova_coefficients))

# Load housing market engine to simulation object
target = s.network
market_mode = 'top_candidate'
s.add_engine(HousingMarket(target, market_mode=market_mode, block_group_sample_size=block_group_sample_size))
...
for t in range(s.network.current_timestep_idx):
    df = s.network.get_history('housing_block_group_df')[t]
...
df = s.network.get_history('housing_block_group_df')[-1]

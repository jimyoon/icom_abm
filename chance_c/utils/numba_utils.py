...
@njit
def fast_market_matching_numba(
    households: np.ndarray,
    geoids: np.ndarray,
    utilities: np.ndarray
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Numba-optimized market matching algorithm.
    
    Args:
        households: Array of household names
        geoids: Array of GEOIDs
        utilities: Array of utility values
        
    Returns:
        Tuple of (assigned_households, assigned_geoids)
    """
    # Create a structured array for sorting
    dtype = np.dtype([('household', 'U50'), ('geoid', 'U50'), ('utility', np.float64)])
    data = np.empty(len(households), dtype=dtype)
    data['household'] = households
    data['geoid'] = geoids
    ...
    assigned_households = np.empty(len(unique_households), dtype='U50')
    assigned_geoids = np.empty(len(unique_households), dtype='U50')
    assignment_count = 0
    
    # Track used geoids with a boolean array
    all_geoids = np.unique(geoids)
    used_geoids = np.zeros(len(all_geoids), dtype=np.bool_)
    
    # Process households in order of their best utility
    for household in unique_households:
        # Find all options for this household
        household_mask = sorted_data['household'] == household
        household_options = sorted_data[household_mask]
        
        if len(household_options) == 0:
            assigned_households[assignment_count] = household
            assigned_geoids[assignment_count] = 'outmigrated'
            assignment_count += 1
            continue
        ...
        for option in household_options:
            geoid = option['geoid']
            # Find index of this geoid in all_geoids
            geoid_idx = np.where(all_geoids == geoid)[0]
            if len(geoid_idx) > 0 and not used_geoids[geoid_idx[0]]:
                assigned_households[assignment_count] = household
                assigned_geoids[assignment_count] = geoid
                used_geoids[geoid_idx[0]] = True
                assignment_count += 1
                assigned = True
                break
    ...
    return assigned_households[:assignment_count], assigned_geoids[:assignment_count]


@njit
def fast_market_matching_batch_numba(
    households: np.ndarray,
    geoids: np.ndarray,
    utilities: np.ndarray,
    batch_size: int = 1000
    ...
    """
    Numba-optimized market matching with batch processing.
    ...
    # Get unique households and their best utilities
    unique_households = np.unique(households)
    household_best_utilities = np.zeros(len(unique_households))
    
    for i, household in enumerate(unique_households):
        household_mask = sorted_data['household'] == household
        if np.any(household_mask):
            household_best_utilities[i] = np.max(sorted_data[household_mask]['utility'])
    
    # Sort households by their best utility
    household_sort_indices = np.argsort(household_best_utilities)[::-1]
    sorted_households = unique_households[household_sort_indices]
    ...
    # Process in batches
    for i in range(0, len(sorted_households), batch_size):
        batch_end = min(i + batch_size, len(sorted_households))
        batch_households = sorted_households[i:batch_end]
...
@njit
def fast_sort_utilities_numba(
    utilities: np.ndarray,
    households: np.ndarray,
    geoids: np.ndarray
) -> tuple:
    """
    Fast sorting of utilities with associated household and GEOID data.
    ...
    Returns:
        Tuple of (sorted_utilities, sorted_households, sorted_geoids)
    """
    # Create indices for sorting
    indices = np.argsort(utilities)[::-1]  # Descending order
    
    # Sort all arrays
    sorted_utilities = utilities[indices]
    sorted_households = households[indices]
    sorted_geoids = geoids[indices]
    
    return sorted_utilities, sorted_households, sorted_geoids 
...
# with normalization
gdf.plot(column='population_change', cmap=cmap, norm=norm, legend=True)
...
#### Plot initial and final population density side-by-side / multi-panel plot example (consistent scale)
# Get min, max, average for color scale
vmin = min(s.network.get_history('housing_bg_df')[0].pop_density.min(), s.network.get_history('housing_bg_df')[-1].pop_density.min())
vmax = min(s.network.get_history('housing_bg_df')[0].pop_density.max(), s.network.get_history('housing_bg_df')[-1].pop_density.max())
vcenter = np.mean([vmin, vmax])
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 7))
s.network.get_history('housing_bg_df')[0].plot(column='pop_density', vmin=vmin, vmax=vmax, cmap='OrRd', ax=ax1, legend=True)
s.network.get_history('housing_bg_df')[-1].plot(column='pop_density', vmin=vmin, vmax=vmax, cmap='OrRd', ax=ax2, legend=True)